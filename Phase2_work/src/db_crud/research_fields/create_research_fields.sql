/*
  ...
  Description: Insert a new research field and associate it with a user.
  Parameters:
    - user_id: The user UUID as a string to associate with
    - research_field: Name of the research field (required, unique)
  
  Returns: research_field_id of the newly created or existing field
*/
...
-- Map user to research field (convert UUID string back to binary)
INSERT INTO UserResearchFields (user_id, research_field_id)
VALUES (UUID_TO_BIN(%(user_id)s), LAST_INSERT_ID())
ON DUPLICATE KEY UPDATE user_id = user_id;

-- Return the ID of the inserted or existing research field
